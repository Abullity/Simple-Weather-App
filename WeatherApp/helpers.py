...
def get_image(epoch, shift):
	local_epoch = epoch - shift
	local_hours = datetime.fromtimestamp(local_epoch).time().hour
	if local_hours < 7:
		image = 'night'
	elif local_hours <= 9:
...
def generate_city(weather_data):
	...
	# openweather api sends epoch as 'dt' and shift in user's timezone in 
	# seconds as 'timezone'
	image = get_image(weather_data['dt'], weather_data['timezone'])
...
def update_city(city):
	...
	city.image = get_image(weather_data['dt'], weather_data['timezone'])
